...
void  redacteur(long no)
{
	...
	// waits to safely read next free position	
	CHECK(sem_wait(&(buf.mutexPos)), "sem_wait() sur mutexPos");
	// checks if sequence was finished by another writer
	if(buf.finished){
		CHECK(sem_post(&(buf.mutexPos)), "sem_post() sur mutexPos");
		break;
	}
	//reads next free position
        i = buf.freePos;
	...
	// writes in buffer
        if (n == MAX_VALUE)
            ...
            buf.buffer[i] = n++;
	// resets positions read count
	buf.nReads[i] = 0;
	
	// prints to terminal the position has been written in 
    ...
    // marks that the sequence has finished
    CHECK(sem_wait(&(buf.mutexPos)), "sem_wait() sur mutexPos");
    buf.finished = 1;
    CHECK(sem_post(&(buf.mutexPos)), "sem_post() sur mutexPos");
    pthread_exit(0);
...
void  lecteur(long no)
{
	...
	if(buf.nReads[i] == NBLECTEURS){
		// tell writers they are free to overwrite this position
